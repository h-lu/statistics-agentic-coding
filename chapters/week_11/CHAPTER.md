...
然后他看到了一篇 2025 年 Kaggle 竞赛获胜方案，用的是随机森林 + XGBoost + 深度学习的三层集成，AUC 0.91。
...
这正好呼应了 Week 09 学的回归假设：线性回归假设 y 和 x 之间是线性关系。如果真实关系是非线性的（比如 y = x²），你可以加多项式特征，但决策树提供了一种更灵活的方式——不需要显式转换特征，树可以自动捕捉非线性关系。
...
这正好呼应了 Week 09 学的过拟合：回归模型如果特征太多、太复杂，也会过拟合。决策树更容易过拟合，因为它可以无限生长，直到"记住"每个样本。
...
2. **特征随机性**：在每个节点，只考虑随机选取的一部分特征（如 `sqrt(n_features)`）
3. **聚合**：多棵树投票
