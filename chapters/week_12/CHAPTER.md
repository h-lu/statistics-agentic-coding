...
这不是孤例。从招聘筛选算法对女性简历的系统性降权，到信用评分模型对低收入群体的隐性歧视，"能预测的模型"不等于"负责任的模型"。欧盟 GDPR 第 22 条常被讨论为与自动化决策说明义务相关，美国 NIST 的 AI 风险管理框架、中国《深度合成管理规定》等治理框架也都在推动同一个方向：**在高风险或受监管场景中，可解释性、公平性和可审计性经常是部署评审中的关键条件**。
...
默认路径：如果你只完成本周主线和基础作业，只需要仓库默认的 `requirements.txt`。
高级路径：如果你要运行 SHAP / fairlearn 的选读示例，先执行 `pip install -r requirements-advanced.txt`。缺少这些可选依赖时，相关示例会退化为简化实现或跳过，不影响主线阅读。
...
1. 理解全局可解释性（特征重要性）与局部可解释性（SHAP，本周重点；LIME 只做认识）的区别
2. 使用 SHAP 值解释单个预测的归因
3. 检测模型中的偏见（数据偏见 vs 算法偏见）
4. 理解基本的公平性指标边界（统计均等、机会均等 Equal Opportunity、均衡机会与均衡错误 Equalized Odds、校准）
...
- 本周新概念（5 个，预算上限 5 个）：
  ...
  4. 公平性指标（Fairness Metrics）：统计均等、机会均等（Equal Opportunity）、Equalized Odds、校准
...
<!-- code_block_exemption: 本章同时覆盖特征重要性、SHAP、公平性与非技术沟通，代码块用于分别演示不同解释路径。 -->
...
"这就是**归因（Attribution）**。"老潘说，"但先别急着把数字翻译成'百分点'。很多 `TreeExplainer` 默认解释的是模型输出空间，常见的是 **log-odds**。例如：基线输出是 0.15（log-odds 空间的基线），`days_since_last_purchase=45` 把输出往流失方向推了 `+0.31`，`purchase_count=2` 推了 `+0.18`，`vip_status=True` 拉回了 `-0.12`。**这不是直接增加 31 个百分点的流失概率。**"
...
*图 1：SHAP 汇总图，既看全局重要性，也看不同样本上的贡献分布。*

![](images/shap_waterfall_plot.png)
*图 2：SHAP 瀑布图，解释单个客户为什么被判为高风险。*
...
"用局部。"老潘说，"但记得先说清楚**解释值对应哪个输出空间**。不要一上来就说'SHAP 值是 0.31，所以概率增加 31%'。更稳妥的说法是：'这位客户最近 45 天没购买，是把模型输出推向流失的一项主要因素；VIP 身份则把风险往回拉。'如果你明确把 explainer 配成概率空间，再谈'概率增加了多少'。"
...
> **监管趋势**：
>
> - **欧盟 GDPR**：第 22 条常被讨论为与自动化决策说明义务、人工干预和申诉权相关
...
> **对你的启示**：
>
> 你本周学的 SHAP 值、特征重要性、公平性评估，不是"可选的附加功能"，而是"负责任建模"的核心部分。**能预测的模型不等于能进入高风险部署评审的模型**——前者可能只看 AUC，后者还要补上可解释性、公平性和审计说明。
>
> 参考（访问日期：2026-02-21）：
...
![](images/group_confusion_matrices.png)
*图 3：分组混淆矩阵，先看不同群体各自的误判结构。*
...
**第二把尺子：Equalized Odds——"各群体被'正确判断'和'错误判断'的结构都尽量接近"**

这把尺子不看"发了多少优惠券"，而是看"判断对不对"。它要求：
- 真正会流失的人，男性和女性被识别出来的比例一样（真阳性率相同）
- 真正不会流失的人，男性和女性被误判为流失的比例一样（假阳性率相同）

问题来了：**如果训练数据中女性样本本来就少，模型很难学出和男性一样好的判断**。这把尺子理论上更全面，但现实中很难同时满足。

如果你**只要求各群体的真阳性率相同**，那才更接近严格意义上的**机会均等（Equal Opportunity）**。本章后面的 TPR + FPR 一起比较，统一称为 **Equalized Odds**，不要和 Equal Opportunity 混用。
...
| **机会均等（Equal Opportunity）** | 各群体的真阳性率相同 | TPR(A=0) = TPR(A=1) | 只约束“不要漏掉真正正例” |
| **Equalized Odds** | 各群体的真阳性率、假阳性率相同 | TPR(A=0) = TPR(A=1)<br>FPR(A=0) = FPR(A=1) | 约束更强，通常更难同时满足 |
| **校准（Calibration）** | 各群体的预测概率与真实概率匹配 | P(Y=1\|Ŷ=p, A=0) = P(Y=1\|Ŷ=p, A=1) | 可能掩盖分配不公 |

"**Equalized Odds**要求模型对男性和女性的真阳性率相同（抓到真正流失客户的比例），以及假阳性率相同（误判为流失的比例）。如果你只盯住 TPR，那是在讲 Equal Opportunity。两者别混。"
...
| 场景 | 优先级 | 推荐公平性指标 |
|------|--------|---------------|
| **信贷审批** | 避免某群体被系统性误拒或误批 | 明确写清是 Equal Opportunity 还是 Equalized Odds，常见做法是至少监控 FPR/TPR 差异 |
| **医疗筛查** | 不要漏掉高风险患者 | 机会均等（Equal Opportunity，优先看 TPR） |
| **营销定向** | 避免分配不公 | 统计均等（各群体获同样多的优惠） |
| **风险评估** | 预测概率准确 | 校准（各群体的概率与真实匹配） |

### 公平性-准确性权衡

老潘打开了一个模拟实验："看，如果我们强制满足统计均等，整体准确率通常会下降，但幅度取决于数据分布、阈值和后处理方法。这个例子里不是固定的 2-3%，有时会更大。"
...
![](images/fairness_metrics_comparison.png)
*图 4：不同公平性指标的对比，重点是权衡而不是口号。*
...
> "本模型使用随机森林，AUC 为 0.89。与基线模型相比，性能差异在所选检验下达到统计显著（p < 0.05）。SHAP 分析显示 `days_since_last_purchase` 在当前解释设置下是主要驱动因素之一（平均绝对 SHAP 值为 0.31，对应的是模型输出空间，不一定是概率百分点）。"
...
| "p < 0.05" | "如果原假设为真，观察到当前或更极端差异的概率低于 5%" |
| "SHAP 值 = 0.31" | "最近购买天数是主要风险驱动因素之一；这个 0.31 先要看它对应的是 log-odds 还是概率空间" |
...
>
> **模型性能**：
> - 模型对高风险和低风险客户的排序区分能力较强（AUC = 0.89）
...
**公平性指标** → 你学会了区分统计均等、机会均等（Equal Opportunity）、Equalized Odds 和校准，理解了它们之间的冲突和权衡。你知道"公平性不是一个'达到/未达到'的指标，而是一个'有多接近'的连续值"。

**非技术沟通** → 你学会了把统计术语翻译成业务语言，但不牺牲严谨性：p 值不再被说成"95% 把握"，AUC 不再被说成"正确识别率"，SHAP 值也不会被直接误译成"31 个百分点的概率贡献"。
...
- 向非技术读者直接堆 p 值、AUC、SHAP 原始数值，既不严谨也容易沟通失败；
...
- [ ] 理解统计均等、机会均等（Equal Opportunity）、Equalized Odds、校准及其权衡
- [ ] 按敏感属性分组评估模型性能
...
更重要的是，你学会了**公平性指标**：统计均等、机会均等（Equal Opportunity）、Equalized Odds、校准。你知道公平性不是单一指标，而是多个相互冲突的目标之间的权衡。没有"完全公平且完全准确"的模型，只有"根据业务场景选择的权衡"。

最后，你学会了**向非技术读者解释**：把 p 值翻译成条件化的证据强弱，把 AUC 翻译成"排序区分能力"，把 SHAP 值翻译成"风险驱动因素"，并先说明解释值所在的输出空间。你知道**业务方不关心你用什么算法，关心的是'我该做什么'**。
