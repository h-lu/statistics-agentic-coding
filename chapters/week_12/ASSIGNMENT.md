...
默认路径：基础题只依赖仓库默认的 `requirements.txt`，即可完成特征重要性、分组公平性和非技术沟通主线。
高级路径：如果你要运行 SHAP / fairlearn 的选读示例或复现全部图表，请先执行 `pip install -r requirements-advanced.txt`。缺少这些可选依赖时，相关示例应当退化为简化实现或跳过。
...
if len(churn_samples) > 0:
    ...
    # 解释这个样本（推荐 waterfall；force_plot 仅作旧版兼容参考）
...
- 写解释时先说明**输出空间**：TreeExplainer 常见的是模型输出或 log-odds，不要把 `0.31` 直接写成“31 个百分点”
...
```
统计术语 → 业务语言：
- "AUC = 0.89" → "模型把高风险客户排在低风险客户前面的区分能力很强"
- "SHAP 值 = 0.31" → "最近购买天数是主要风险驱动因素之一；这个 0.31 要先说明是在 log-odds 还是概率空间"
...
模型性能：
模型对高风险和低风险客户的排序区分能力较强（AUC = 0.89）。在 100 个真实会流失的客户中，模型能抓到 72 个。
...
> "本模型使用随机森林，AUC 为 0.89，说明模型排序区分能力较强。SHAP 分析显示 days_since_last_purchase 是主要驱动因素之一（SHAP 值 0.31），其次是 purchase_count（SHAP 值 0.18）。模型对男性和女性的准确率都是 0.86，说明模型不存在性别偏见。"
...
```
（用你自己的话写，修正上述问题）
例如："AI 的解释有两个问题：第一，SHAP 值 0.31 没有解释输出空间，不能直接写成概率百分点。第二，准确率相同不代表公平，应该看真阳性率和假阳性率的差异。"
```
