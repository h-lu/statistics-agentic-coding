...
-->
<!-- code_block_exemption: 本周需要并行展示相关分析、分组比较、时间序列和假设清单，代码块数量略超 10 个是刻意保留的示例密度 -->
...
如果你想把假设写得更像一份可以交给同事审核的任务单，再补三项会更完整：
- **控制变量**：哪些因素需要先固定住，避免混淆
- **后续方法**：下周或后续章节准备用什么检验
- **风险**：如果假设不成立，最可能的解释是什么
...
不是每张图都应该直接变成结论。更好的做法是，把图表拆成三层：

1. **Description**：我看见了什么？
2. **Inference**：这可能说明了什么？
3. **Prediction**：接下来可以检验什么？
...
这样做的好处是，图表不再只是“展示结果”，而是直接变成下一步分析的入口。
