...
IQR（interquartile range，四分位距）的公式很简单：

$$
IQR = Q_3 - Q_1
$$

其中 $Q_1$ 是第 25 百分位数（也叫第一四分位数），$Q_3$ 是第 75 百分位数（第三四分位数）。IQR 描述了"中间 50% 的数据"分布有多宽，对极端值不敏感。
...
>
> pandas 的 `.var()` 默认使用 **n-1**（无偏估计，样本方差），而 numpy 的 `np.var()` 默认使用 **n**（总体方差）。
>
> ```python
> import pandas as pd
> import numpy as np
> data = [1, 2, 3, 4, 5]
> print(f"pandas: {pd.Series(data).var():.2f}")      # 2.50（除以 n-1=4）
> print(f"numpy:  {np.var(data):.2f}")               # 2.00（除以 n=5）
> ```
>
> 对样本数据（绝大多数情况），使用 pandas 的默认值更合适——它提供对总体方差的无偏估计。
...
想象两款产品的月营收都差不多，平均都是 10 万元。产品 A 每个月都在 9.8 万到 10.2 万之间，像一条平滑的线；产品 B 有时 6 万，有时 14 万，平均值看起来一样，但经营体验完全不同。**标准差说的就是这种"稳不稳"的差别**。如果你只看均值，就会把这两款产品误判成"一样"。
...
偏度描述分布的"不对称性"：负偏态表示左边有长尾，正偏态表示右边有长尾。峰度描述分布的"尖峭程度"：峰度高表示中间更尖、尾部更厚，峰度低表示更平坦。**注意**：pandas 的 `kurtosis()` 返回 excess kurtosis（超额峰度），正态分布的峰度值为 0（而非 3）。
...
箱线图的"须"（whiskers）通常延伸到 **Q1-1.5×IQR 和 Q3+1.5×IQR**，但也可能停在数据的实际最小值或最大值（取决于 seaborn 的实现）。具体来说：
- 须的内边界是 Q1-1.5×IQR 和 Q3+1.5×IQR
- 须的实际终点是 **不超过内边界** 的最远数据点（可能是 min/max）
- 超出内边界的点会被画成单独的点——它们可能是异常值
...
![](images/area_trap_demo.png)
*图：面积陷阱演示。同一 10% 的差异，饼图（左）看起来比柱状图（右）大得多——人类对"面积"的感知不如对"高度"准确*
...
AI 会。如果你让 AI "画一个好看的柱状图"，它可能会截断 Y 轴让差异更明显，或者用鲜艳的颜色让图表"更吸引眼球"。**人类可能有意误导读者，AI 可能无意"讨好"用户——两者生成的图表都需要你亲自审查。**
...
如果你想把这部分写得更模块化，可以把"单列统计"拆成 `summarize_numeric_column()`，把"图表输出"拆成 `plot_distribution_with_stats()`，再用 `build_one_page_report()` 把二者串起来。
...
![](images/one_page_report.png)
*图：一页分布报告示例。包含摘要统计表、直方图、箱线图，是本周 StatLab 的最终交付物*
