...
### 公式补充（已添加）
- [x] 标准差公式：$s = \sqrt{\frac{1}{n-1}\sum(x_i - \bar{x})^2}$（使用样本标准差符号 $s$）
- [x] IQR 公式：$IQR = Q_3 - Q_1$

### 第 7 轮技术修复（2026-02-20）
- [x] ~~CHAPTER.md 第 166-173 行 pandas `agg()` 使用了错误的嵌套元组语法~~ → 已修正为正确的 keyword + lambda 语法
- [x] ~~CHAPTER.md 第 201 行标准差公式使用 $\sigma$（总体符号）但公式为样本标准差~~ → 已改为 $s$ 并添加说明
...
| 第 7 轮 | 18/20 | 修复 S2 pandas agg() 语法、S3 标准差公式符号，release 校验通过 |
