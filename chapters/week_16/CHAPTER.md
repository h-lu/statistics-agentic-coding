...
- 建议示例文件：examples/16_report_pipeline.py（报告生成入口脚本）、examples/16_audit_checklist.py（审计清单脚本）
-->
...
| **环境** | 依赖与版本 | "我电脑上能跑" | `requirements.txt`；如使用 conda，可额外提交自建 `environment.yml` |
...
flowchart LR
    ...
    B --> C[analysis]
    C --> D[figures / report.md / report.html]
    C --> E[ai_review_log]
    E --> D
```

这张图的重点很简单：**数据要先清洗，再分析，再生成图表与报告；如果用了 AI，也要把审查记录回流到报告交付链里**。
...
核心改动：
1. 创建一个入口脚本（可命名为 `generate_report.py`，也可复用 `examples/16_final_delivery.py` 的结构）
2. 把每周的分析函数按顺序调用
...
核心改动：
1. 可选：创建 `report_template.md` 作为模板；也可直接使用 Python f-string/Jinja 模板
...
终稿检查时，审计清单至少要覆盖这七件事：**数据来源、清洗决策、诚实图表、不确定性、模型诊断、因果表述、AI 使用记录**。
...
| 环节 | 占时 | 核心问题 | 常见错误 |
|------|------|----------|----------|
| **问题** | 45 秒 | 为什么要做这个分析？ | 花太多时间介绍背景数据 |
| **方法** | 60 秒 | 你怎么分析？用什么方法？ | 罗列技术细节，听众跟不上 |
| **发现** | 120 秒 | 你发现了什么？ | 只放数字，不放图表 |
| **边界** | 45 秒 | 这个结论的局限是什么？ | 避而不谈，显得不诚实 |
| **反思** | 30 秒 | 这个分析意味着什么？ | 没有回到问题，缺少收束 |
...
**坏的开头**：
> "我用 pandas 读取数据，用 scikit-learn 训练了一个随机森林模型，然后用模型系数做可解释性分析；如已安装 SHAP，可追加 SHAP……"
...
**示例**：
> "我们分析了 5000 个客户的行为数据，包括使用时长、消费金额、客服联系次数等 15 个变量。我们用了逻辑回归预测流失，用 模型系数解释；如已安装 SHAP，可追加 SHAP 分析。在分析前，我们检查了数据的缺失机制——缺失率最高的字段是'最后一次登录'，但我们判断是 MAR（随机缺失），所以做了多重插补。"
...
**示例结构**：
> "这张图是我们最重要的发现（指向 ROC 曲线）。模型的 AUC 约为 0.78，这意味着如果我们用模型识别前 20% 高风险客户，能更集中地找到高风险客户（覆盖率需用当前数据实际计算）。
>
> 更重要的是，模型系数/可选 SHAP 分析提示：'使用时长'和'客服联系次数'是重点关注变量——这给我们一个可操作的建议：主动联系低活跃客户。"
...
3. **假设条件**：模型的前提假设满足了吗？（回顾 [模型诊断] week_09）回归模型有 LINE 假设，逻辑回归需要样本独立、logit 线性、事件数充足，并避免完全分离等问题
...
> - **推断**：特征与流失的关联强度（报告系数、置信区间或 pseudo-R²，并说明分类指标的含义）
> - **预测**：新客户的流失概率（AUC ≈ 0.78，区分能力良好）
>
> 三个可操作建议：
> 1. **优先干预**：模型识别出的前 20% 高风险客户，应报告其实际覆盖的流失者比例
...
| 差展示 | 好展示 |
|------|------|
| 127 页 PPT，图很多但没有主线 | 5-7 页，每页只讲一个问题 |
| 图表堆在一起，没有解释 | 每张图后面都写"应该看到什么" |
| 只讲结论，不讲边界 | 先讲发现，再讲假设和限制 |
| AI 直接生成整套幻灯片 | AI 先给建议，人类负责审查与定稿 |
...
建议直接参考 `starter_code/ai_usage_log_template.md`，把必填字段先填成表格，再补充文字说明。
