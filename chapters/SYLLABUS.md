...
- 大数据的挑战：维度灾难、计算复杂度
- 降维技术：PCA（以 t-SNE、UMAP 作为补充比较）
- 聚类分析：K-means、层次聚类
- 聚类稳定性与结果解释
- 在 StatLab 报告中加入高维可视化与客户分群模块
...
- **命名口径统一**：跨周的 StatLab 核心产物统一叫 `report.md`（可选导出 `report.html`）；各周 `examples/*statlab*.py` 只是示例脚本名，不是强制提交文件名。
...
| 因果与扩展 | 13-15 | 因果图 + 贝叶斯视角 + PCA/聚类模块 |
| 展示 | 16 | 终稿报告（含展示版） |
