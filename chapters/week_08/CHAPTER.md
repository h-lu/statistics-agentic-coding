...
<!-- code_block_exemption: 本章需要分开保留点估计、CI、Bootstrap、置换检验和 StatLab 示例的独立代码块，便于读者逐步跟随；合并会削弱可读性和教学节奏。 -->
...
| 常见误解 | 为什么错 | 正确说法 |
|---|---|---|
| "95% CI 意味着真实均值有 95% 的概率落在区间里" | 频率学派里参数是固定的，不是随机的 | 95% CI 的意思是：重复抽样很多次，类似这样构造的区间大约有 95% 会包含真值 |
| "95% CI 就是 95% 数据都落在这个范围里" | 这是在说数据分布，不是在说参数估计 | CI 讲的是参数不确定性，不是原始观测值的覆盖率 |
| "CI 不包含 0 就说明差异一定很大" | 统计显著不等于实际重要 | CI 不包含 0 只说明在当前模型和假设下，零值不太支持；大小还要看业务尺度 |
| "CI 越窄就一定越正确" | 区间窄不代表没有偏差 | CI 变窄通常说明样本量更大或波动更小，但如果样本有偏，区间依然可能很误导 |

阿码有点晕："那我在报告里应该怎么写？"

"你可以说：'我们构建了 95% 置信区间 [2.8, 3.6]。在重复抽样的情况下，类似这样的区间大约有 95% 会包含真实的均值。'——或者更简洁一点：'均值估计为 3.2，95% CI [2.8, 3.6]。'"
...
```mermaid
flowchart TD
    A[我想要一个区间或一个检验结论] --> B{数据和问题大致符合正态/公式假设吗?}
    B -->|是| C[t 区间 / 理论公式]
    B -->|否| D{我更关心哪一种?}
    D -->|区间估计| E[Bootstrap CI]
    D -->|两组差异检验| F[Permutation Test]
    D -->|我想表达先验或后验概率| G[Bayesian / Credible Interval]
    C --> H[快、直观，适合简单场景]
    E --> I[通用，适合非正态或复杂统计量]
    F --> J[不依赖分布假设，适合置换思路]
    G --> K[选读，Week 14 详细讲]
...
老潘的经验法则：
- **简单均值、正态近似够用** → `t` 区间；
- **分布难推导、统计量复杂** → Bootstrap；
- **两组差异、偏非参数思路** → Permutation；
- **想谈参数的概率** → Bayesian（选读）。
