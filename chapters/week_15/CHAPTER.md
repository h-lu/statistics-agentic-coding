...
下面这组值来自脚本中的固定随机种子，展示的是同一批随机点对之间的最小/最大距离差异：

| 维度 | 最小距离 | 最大距离 | 距离范围 | 变异系数 |
|------|---------:|---------:|---------:|---------:|
| 2 维 | 0.0010 | 1.3506 | 1.3496 | 0.4768 |
| 10 维 | 0.2308 | 2.3064 | 2.0756 | 0.1925 |
| 50 维 | 1.7976 | 3.8873 | 2.0898 | 0.0842 |
| 100 维 | 2.8983 | 5.0927 | 2.1944 | 0.0590 |

在 100 维空间中，距离范围和变异系数都明显收缩——**这意味着"最近"和"最远"已经越来越接近，距离不再像低维那样有辨别力**。
...
![维度灾难示意](images/01_curse_of_dimensionality.png)
这张图强调了高维空间里数据点会越来越稀疏，距离感知也会越来越弱。

![最近邻失效示意](images/01_nearest_neighbor_failure.png)
这张图把"最近邻"和"最远邻"的差距缩小过程画出来，说明高维下距离指标为什么不再可靠。
...
![K-means 评估示意](images/03_kmeans_elbow_silhouette.png)
这张图把肘部法则和轮廓系数放在一起，提醒你选 K 不能只看单一指标。
...
![层次聚类树状图](images/04_hierarchical_dendrogram.png)
这张树状图展示了簇的合并过程，适合用来观察不同层级上的客户分组结构。
...
前面我们用肘部法则和轮廓系数选 K。选定之后，最好再看一眼二维空间里的簇分布，确认模型没有把明显重叠的数据硬切成几块。

![K-means 二维聚类图](images/03_kmeans_2d_clusters.png)
这张图展示了 K-means 在二维特征空间里的分组结果，便于快速检查簇边界是否过于生硬、是否存在明显的混叠区域。

> **完整代码见**：`examples/03_kmeans_demo.py`

老潘提醒："**如果二维图上已经挤成一团，就不要对 K-means 的分群边界讲得太满**。它给的是结构线索，不是绝对真相。"
...
![PCA+聚类散点图](images/04_pca_cluster_overlay.png)
这张图把聚类结果叠加到 PCA 平面上，方便你直接看出不同簇在二维空间中的分布边界。
...
> - 先让 AI 猜一个 K，再用肘部法则、轮廓系数和稳定性检查去验证或反驳它
> - 这些簇对应哪些客户群体？
> - 我们应该对每个簇采取什么行动？
> - 聚类结果是否稳定、是否可复现？
>
...
![累积方差图](images/02_pca_cumulative_variance.png)
这张图让读者一眼看到保留多少主成分才能覆盖大部分方差，避免把降维误读成"无损压缩"。
...
![载荷热力图](images/02_pca_loadings_heatmap.png)
这张图把每个原始特征在主成分上的贡献摊开，方便把抽象成分翻译成业务术语。
...
![PCA 二维散点图](images/02_pca_2d_scatter.png)
这张图把高维客户行为压到二维平面，帮助你直观看到流失风险、活跃度和价格敏感度的空间分布。
...
![聚类稳定性示意](images/04_cluster_stability.png)
这张图提醒你，聚类不只要看一次结果，还要看重采样或重复运行后是否稳定。
