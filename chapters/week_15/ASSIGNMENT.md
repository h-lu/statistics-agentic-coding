...
**建议统一的报告字段**：
- 输入数据：数据来源、样本量、特征维度
- 特征选择理由：为什么保留这些变量、删除哪些变量
- 标准化方式：是否做 `StandardScaler`、为什么
- 主成分解释：前几个主成分分别代表什么业务含义
- 聚类数选择：K 值如何确定，是否结合 elbow / silhouette
- 稳定性检查：Bootstrap、不同随机种子、ARI 等
- 非技术解释：给业务方的简短说明，不用术语堆砌
