...
你有三组用户数据（A、B、C 三个渠道的客单价/消费金额，连续型指标）。请执行单因素 ANOVA，判断三组均值是否有显著差异。

**数据**（在 `starter_code/assignment_data.py` 中提供）：
```python
# 三组渠道的客单价数据（连续型指标，单位：元）
channel_a = [98.4, 112.1, 87.6, 104.3, 95.2, 118.0, 101.5, ...]
channel_b = [105.7, 121.4, 99.8, 110.2, 116.9, 103.3, 128.1, ...]
channel_c = [119.3, 132.5, 108.8, 125.6, 114.2, 136.1, 122.4, ...]
```
...
A vs B  | +7.4     | [-1.8, 16.6] | 0.12       | ❌
A vs C  | +19.2    | [10.1, 28.3] | 0.01       | ✅
B vs C  | +11.8    | [2.7, 20.9]  | 0.02       | ✅

显著比较：
- A vs C：p = 0.01，C 渠道客单价比 A 渠道高 19.2 元 [10.1, 28.3]
- B vs C：p = 0.00，C 渠道客单价比 B 渠道高 11.8 元 [2.7, 20.9]
```
...
实际意义：组别可以解释 xxx% 的客单价变异，其余 xxx% 来自其他因素。
```
...
使用 `starter_code/challenge_data.csv`（包含 5 个渠道、1000 个用户的客单价数据），完成一份完整的多组比较报告。

**要求**：

1. **数据探索**（可视化）
   - 画一张箱线图，展示 5 个渠道的客单价分布
   - 画一张条形图，展示 5 个渠道的平均客单价（带误差线）
...
- [ ] **检验方法选择**：AI 使用了什么检验？是否合适？
  ...
  - 对于二元转化率，不应直接套用 ANOVA/Tukey；应使用比例检验、卡方检验或逻辑回归，并配合 Bonferroni / Holm / FDR 等多重比较校正
...
**提示**：
- 使用本周学到的知识：多重比较问题、ANOVA + Tukey HSD（连续指标）、比例检验/卡方检验（二元转化率）、校正方法
