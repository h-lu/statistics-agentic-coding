...
> **FDR 的直觉（了解即可）**
>
> FDR 的核心思路是：**先把 p 值从小到大排序，再找出最大的 k，使得第 k 个 p 值还满足 BH 条件 `p_(k) <= (k/m) * α`**。然后把前 k 个都保留下来。
>
> 比如有 20 个假设，按 p 值从小到大排序后：
> - Bonferroni 要求每个 p 值都 < 0.05/20 = 0.0025 才算显著
> - FDR 可能发现"前 8 个 p 值都比较小"，于是把前 8 个都保留下来——即使第 8 个的 p = 0.015（> 0.0025）
>
...
```mermaid
flowchart TD
    A[开始：我要做多重比较] --> B{检验次数多吗?}
    B -->|少于 10 次| C[Bonferroni 或 Tukey HSD]
    B -->|10 到 50 次| D{研究是探索性还是确认性?}
    D -->|探索性| E[FDR (BH)]
    D -->|确认性| F[Bonferroni]
    B -->|超过 50 次| E
    C --> G[适合：少量比较、尽量别报假阳性]
...
老潘把这个流程压成一句话：
- **少量比较**，优先 Bonferroni 或 Tukey HSD；
- **很多比较**，优先 FDR；
- **ANOVA 后的两两比较**，默认 Tukey HSD。
