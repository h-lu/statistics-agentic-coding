...
> **FDR 算法的原理（了解即可）**
>
> Benjamini-Hochberg 的核心思路是：
> 1. 将所有 p 值从小到大排序：p₁ ≤ p₂ ≤ ... ≤ pₘ
> 2. 找到最大的 k，使得 pₖ ≤ (k / m) × α
> 3. 拒绝前 k 个假设
>
> 你不需要手写这个算法——`statsmodels.stats.multitest.multipletests` 已经实现了它。你只需要理解：**FDR 比Bonferroni 更"宽容"，在检验次数多时更有可能发现真实效应**。
