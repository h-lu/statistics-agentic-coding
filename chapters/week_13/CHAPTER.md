...
-->
<!-- code_block_exemption: 本章把因果图、RCT、观察研究和 StatLab 模块拆开演示，代码块数量因此偏多。 -->
...
![RCT 和观察研究对比](images/rct_vs_observational.png)
这张图把随机对照试验和观察研究并排放在一起，强调了随机化为什么能切断混杂路径，而观察研究为什么必须依赖额外假设。
...
![混杂结构示意](images/dag_confounding.png)
这张图对应最常见的混杂情形，提醒你不要把"相关"误当成"因果"。

![碰撞结构示意](images/dag_collider.png)
这张图说明为什么控制碰撞变量会反而打开一条假的关联路径。

![中介结构示意](images/dag_mediation.png)
这张图帮助区分总效应和直接效应，也提醒你别把中介当成普通控制变量。
...
## 5. 观察研究中的因果推断（选读拓展）——当 RCT 不可行时

小北本周最后的问题是："**公司不可能对所有决策都做 A/B 测试，有些决策已经实施了，怎么事后评估因果效应？**"

老潘的答案是："**观察研究中的因果推断**——用统计方法逼近因果答案。"

这一节属于本周的**拓展阅读**：如果你已经把前四节的因果图、d-分离和 RCT 金标准吃透，再回来看会更顺手。这里我们介绍两个最常用的方法：双重差分（DID）和工具变量（IV），并顺带提到倾向得分匹配（PSM）作为补充思路。
...
![DID 平行趋势示意](images/did_parallel_trends.png)
这张图是检验 DID 的第一道门槛：政策前两组趋势是否大致平行，决定了后面的差分能不能被解释成因果效应。
...
![倾向得分匹配示意](images/psm_matching.png)
这张图把处理组和对照组在匹配前后的重叠关系画出来，直观展示了 PSM 的目标是让可观测协变量更平衡。
