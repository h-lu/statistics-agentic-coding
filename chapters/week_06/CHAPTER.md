...
**一句话正确定义**：p 值 = P(在原假设为真时，观察到与当前数据**同样或更偏离原假设**的数据)

用人话说就是：**假设真实没有差异，你观察到的这个差异有多罕见？**"更偏离"指的是比观察值差异更大——无论是正方向还是负方向。
...
**完全正确。** p 值的本质是"排名"：如果你做的是单尾检验，观察值排在零假设分布某一端最极端的 2%，单尾 p 值约为 0.02；如果做双尾检验，还要同时计入另一端同样极端的区域，双尾 p 值通常约为 0.04。
...
> ⚠️ **注意**：转化率数据是二元的（0/1），严格来说不满足 t 检验的正态性假设。这里先用 t 检验演示流程，但**实际工作中应优先使用比例检验**（后文会介绍 `proportions_ztest`）。当样本量足够大时（每组 > 30），t 检验的结果与比例检验通常接近，因为中心极限定理保证了均值的抽样分布接近正态。
...
if p_value < 0.001:
    ...
    print("结论：统计上显著（p < 0.05），但不是高度显著；效应量只有", interpretation)
```

运行后你会发现：p ≈ 0.016（在 0.05 水平上统计显著，但不是 p < 0.001 这种"高度显著"），同时 Cohen's d ≈ 0.1（小效应）。这意味着：**这个差异在统计上"不太可能是运气"，但在实际意义上"差异很小"**。
...
假设检验的前提检查，就像是给数据做**体检**：Q-Q 图是 X 光片，一眼看出数据"长得正不正"；Shapiro-Wilk 检验是血液检测，给你一个数值化的"健康指标"。两种方法结合使用，才能做出准确判断。
...
> ⚠️ **老潘的提醒**：当样本量很大时（n > 5000），正态性检验会过于敏感——即使轻微偏离正态，p 值也会很小。这时要结合 Q-Q 图和实际判断，不要只看 p 值。
...
阿码看完代码，心想"检查这么多，应该没必要吧"。他找到一组历史数据，直接跑了个 t 检验，p = 0.03，开心地跑去找老潘："你看！显著差异！"

老潘只看了一眼，就问："正态性检查了吗？"

阿码愣住了："呃……没有。"

老潘让他画了个 Q-Q 图。结果发现数据严重右偏——这是一个收入数据集，少数高收入用户拉长了尾巴。用 Shapiro-Wilk 检验，p < 0.001，明确拒绝正态性。

"现在用 Mann-Whitney U 检验重跑一次。"

阿码重跑后，p = 0.12。不显著。

"这就是**假设检查的价值**。同一个数据，检查假设之前是'显著'，检查之后是'不显著'。如果你直接用 t 检验的结论做决策，就是用错误的工具得出错误的结论。" 老潘总结道。

阿码默默记下了一条规则：**跑检验之前，先检查前提假设**。
...
> **📋 快速检查：审查 AI 报告的 5 个关键问题**
>
> 看到一份 AI 生成的统计报告，问自己这 5 个问题：
>
> 1. **前提假设检查了吗？** → 检验方法是否适合数据类型？
> 2. **效应量报告了吗？** → p < 0.05 只说明"不是运气"，效应量说明"差异多大"
> 3. **置信区间给出了吗？** → 区间比单点 p 值更诚实
> 4. **多重比较问题考虑了吗？** → 只报告显著的 = cherry-picking
> 5. **结论是否过度解读？** → 统计显著 ≠ 商业决策
