...
小北看到准确率 84.67%，很高兴。但老潘问了一句："你的数据里 80% 没流失，如果你的模型永远预测'不流失'，准确率也是 80%。你的模型比'永远猜不流失'好多少？"
...
阿码看了你的结果，说："等等，数据里 80% 不流失，20% 流失。如果我的模型永远预测'不流失'，准确率也有 80%。你的模型 84.67%，真的比'傻瓜基线'好吗？"
...
1. 绘制 ROC 曲线（测试集）
2. 计算 AUC
3. 在一张图上画出：你的模型 ROC 曲线 + 随机猜测对角线（可选：再加上 DummyClassifier 作为工程 baseline）
...
**输出示例**：
```
交叉验证 AUC: 0.8278 (+/- 0.0294)
各折 AUC: [0.8845 0.8017 0.8100 0.8183 0.8242]
```
...
- 如果你看到 AUC 异常接近 1.0，要优先排查目标泄漏或未来信息泄漏
...
> "基于逻辑回归模型，我们准确预测了 84.67% 的客户流失情况。模型表现优秀，可以投入生产使用。特征 importance 显示购买次数是最重要的预测因素。"

**审查清单**：
- [ ] 它有没有检查类别不平衡？（准确率 84.67% 可能没有意义）
- [ ] 它有没有说明选择了哪个评估指标？为什么？
...
```
（用你自己的话重写，修正上述问题）
例如："模型准确率 84.67%，略高于 80% 的多数类基线，但召回率只有 33.33%，说明仍漏掉了不少流失客户。混淆矩阵显示模型漏判了 40 个流失客户（FN），误判了 6 个不流失客户（FP）。AUC 为 0.81，表明模型具备一定排序能力，但上线前还应继续调阈值、检查泄漏风险并补充更强特征。"
```
