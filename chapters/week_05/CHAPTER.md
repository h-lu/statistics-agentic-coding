...
| 标准差 SD | 标准误 SE |
|---|---|
| 描述原始数据点围绕均值的波动 | 描述统计量围绕真实值的波动 |
| 数据越分散，SD 越大 | 样本量越大，SE 往往越小 |
| 常用于描述"这批数据散不散" | 常用于描述"这个均值有多稳" |

这张对比卡很重要，因为很多报告会把这两个词混着写。你可以记成一句话：**标准差看数据，标准误看结论。**
...
### 现实数据例子：30 个客户 vs 200 个客户

如果你想看"平均消费"到底稳不稳，最直接的办法不是只看一个总均值，而是把 `week_05_data.csv` 里的 `revenue` 列拿来做两种抽样：
...
- 30 个客户的平均消费更容易上下晃动，波动看起来更大；
- 200 个客户的平均消费更集中，结果更稳定。

这不是抽样技巧的花活，而是标准误在现实里的样子。**样本越大，均值越稳，SE 越小。**
...
更完整的数学推导见 `examples/appendix_sampling_math.md`（选读）。正文里我们只保留结论和直觉，避免把主线讲成公式墙。
