...
**任务**：用模拟观察"总体分布长什么样"时，样本均值分布如何逐步靠近正态。

**基础题（5 分）**
1. 生成一个高度右偏的分布（如指数分布）
2. 只看 `n=30` 的样本均值分布
3. 用一句话解释：为什么它还没有完全变成正态？

**进阶题（5 分）**
1. 分别取样本量 `n=5, 30, 100`
2. 重复抽样 1000 次，画出不同样本量下的样本均值分布
3. 回答：哪个样本量开始明显接近正态？为什么？

**挑战题（5 分）**
1. 比较至少三种总体分布（如指数、均匀、正态）
2. 对每种总体都画出 `n=5/30/100` 的样本均值分布
3. 回答：总体形状会怎样影响 CLT 的收敛速度？
...
- 总体分布：指数分布（高度右偏）
- 样本量：30
- 现象：样本均值分布已经比原始分布更对称，但仍可能保留一点偏态
...
### 结论
总体越偏态，样本均值分布越需要更大的样本量才能接近正态。这就是中心极限定理的直觉。
```
