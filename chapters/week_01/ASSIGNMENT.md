...
2. **生成数据卡**：编写一个函数 `generate_data_card(df, metadata)`，生成包含以下内容的 Markdown 格式数据卡：
   - 数据来源（`dataset_name`, `source`, `license_or_terms`, `collection_process`, `unit_of_analysis`）
   - 规模概览（`row_count`, `column_count`）
   - 缺失概览（`missing_summary`，哪些字段有缺失、缺失数量和比例）
   - 已知限制（`known_limitations`, `not_suitable_for`, `human_review_notes`）
   - 字段字典（字段名、pandas 类型、统计学类型、缺失率）
...
请你做三件事：
1. **把数据事实和来源事实分开**：哪些信息来自数据本身，哪些必须查来源文档才知道？
2. **检查样本代表性**：这份数据能代表什么，不能代表什么？
3. **指出不能编造的字段**：例如许可证、收集过程、适用范围，哪些必须空着并标注"待核实"？
...
从 Week 05 开始，我们会逐步引入更完整的 AI 审查练习，教你如何评估 AI 生成的统计分析结论是否可靠。在此之前，先建立自己的数据卡审查直觉。
