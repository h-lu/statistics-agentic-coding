...
统计学家 Andrew Gelman 曾指出："统计学不是计算，是提问。"在你写任何代码之前，先搞清楚你的分析属于哪一类：
...
不是。更准确的说法是：**明确你在回答哪一类问题**。不要算了一个均值（描述），就立刻下结论说"总体就是这样"（推断），也不要跑了一个回归（预测），就以为你理解了变量之间的关系（推断）。
...
>
>
> **延伸阅读**：如果你想体验"AI 自动跑 EDA"，可以试试 [ydata-profiling](https://github.com/ydataai/ydata-profiling)（原 pandas-profiling）——一行代码生成完整的数据分析报告。但记住：它能告诉你"数据长什么样"，却不会问你"这份数据能不能回答你的问题"。
...
- **离散（Discrete）**：计数型，只能取整数值，如孩子数量、访问次数
...
阿码眼睛一亮，突然问："那如果我把 species 转成整数（Adelie=0, Chinstrap=1, Gentoo=2），会不会有什么问题？比如 0、1、2 这个顺序会不会影响结果？"

这是个好问题——但也暴露了他还没真正理解"类型"的语义。当你把物种编码成 0、1、2 时，计算机并不知道这些数字只是标签。如果你不小心用它算了个"平均物种"——结果是 0.92——这个数字看起来很精确，但没有任何意义。更关键的是，如果下周来了新数据，出现第四种物种，你的编码体系就崩了。
...
你可以先用 AI 生成候选，但**必须逐项人工核实**——AI 不知道你的业务背景：
