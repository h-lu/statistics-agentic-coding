...
斜率：在这个线性模型中，喙长度每增加 1 mm，喙深度的预测均值变化 XX.XX mm
...
- R² 高不等于假设满足
- R² 高不等于因果关系
- 本周只做样本内拟合和诊断；泛化能力评估会在 Week 10 系统展开
...
- p > 0.05 表示"不能拒绝"对应假设，不等于证明假设成立；要结合图形判断
...
**提示**：
- 在多元回归中，系数表示"控制其他变量后"的条件关联
- 如果自变量之间相关，简单回归的系数可能混入其他变量的关系
- 多元回归的系数不是天然"因果影响"，只能在模型和变量控制条件下解释
...
假设你让 AI 分析"广告投入与销售额的关系"，它给你生成了下面这份结论。请审查它，找出其中的问题。
