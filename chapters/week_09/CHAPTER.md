...
"R² 这个数字是真的，但它不能证明模型假设成立。"老潘继续说，"你有没有检查残差？有没有验证线性假设？有没有考虑异常值对结论的牵动？"
...
老潘点头："你已经在回答'有没有差异'这个问题。接下来要问的是：'这个差异和哪些变量有关？在控制其他变量后，这种关系还在吗？'"
...
1. 理解回归分析的本质——既可用于预测，也可用于理解变量关系的统计模型
2. 解释回归系数的含义，并用回归分析回答"X 与 Y 有什么关联、在控制其他变量后关系如何"
3. 检查回归模型的假设（LINE：线性、独立性、正态性、等方差）
4. 诊断模型问题：残差图、QQ图、Cook's 距离、VIF（多重共线性）
5. 在 StatLab 报告中写出"带诊断检查"的回归结论

本周的重点是**解释与诊断**：先把样本内的线性关系、系数含义、假设检查和模型局限讲清楚。回归当然也能用于预测，但训练/验证划分、交叉验证和泛化评估会在 Week 10 系统展开。

## 互动页面（课堂演示）

本周配套 5 个可离线打开的独立 HTML 互动页面，建议按课堂进度穿插使用：

1. [OLS Line Lab：为什么这条线是“最好”的？](interactive/01_ols_line_lab.html) — 拖动散点，观察 OLS 回归线、残差、SSE 和 R² 如何变化。
2. [Residual Diagnostics Lab：残差图诊断诊所](interactive/02_residual_diagnostics_lab.html) — 切换线性、非线性、异方差和高影响点场景，练习模型诊断。
3. [R² Prediction Trap：R² 高就一定好吗？](interactive/03_r2_prediction_trap.html) — 比较训练集 R² 与测试表现，为 Week 10 的泛化评估铺垫。
4. [Multicollinearity VIF Lab：自变量“打架”时发生什么？](interactive/04_multicollinearity_vif_lab.html) — 调节自变量相关性，观察 VIF 和系数解释风险。
5. [Interaction Terms Explorer：交互项不是“再加一个变量”](interactive/05_interaction_terms_explorer.html) — 调节两组斜率和截距，理解 `x:group` 的含义。

> 使用方式：这些页面都是单文件 HTML，无需联网或构建；在浏览器中打开对应文件即可课堂演示。
...
认知负荷预算：
- 本周新概念（5 个，预算上限 5 个）：
  1. 简单线性回归（simple linear regression）：y = β0 + β1x + ε
...
- 建议示例文件：examples/99_statlab_regression.py（本周报告生成入口脚本）
-->
<!-- code_block_exemption: 本章将回归原理、诊断图、StatLab 示例和互动页拆开讲解，代码块数量因此超过默认上限。 -->
...
但相关有个局限：它只告诉你"关系有多强"，不告诉你"y 会随着 x 变化多少"。你说广告投入和销售额"强相关"（r = 0.75），但决策者问的是："我多投 10 万广告，能多卖多少？"相关回答不了这个问题。

**回归分析**（regression analysis）能回答这个问题。它不仅告诉你"有没有关系"，还告诉你"y 如何随 x 变化"——量化的关系。

回归方程是这样的：y = β0 + β1x + ε

- β0 是**截距**（intercept）：x = 0 时 y 的预测值
- β1 是**斜率**（slope）：x 每增加 1 单位，y 的变化量
...
# 画散点图 + 回归线
# sns.regplot 会自动绘制散点图并拟合回归线
plt.figure(figsize=(8, 6))
sns.regplot(x=ad_spend, y=sales, line_kws={'color': 'red'})  # line_kws 设置回归线颜色
plt.xlabel('广告投入（万元）')
plt.ylabel('销售额（万元）')
...
2. **预测**：给定 x，预测 y 是多少？
3. **关系检验**：在线性模型和假设条件下，x 与 y 的线性关系是否显著不为 0？（系数的 p 值、置信区间）

小北若有所思："所以我上周算的相关系数，只是第一步？"

"对。"老潘说，"相关告诉你'有关系'，回归告诉你'什么关系'。而且，回归还能告诉你'在当前模型假设下，这个关系是否显著'——这就要看 p 值了。"

这正好引出下一节的内容：如何解读回归系数和 R²？什么算"显著"？

![](images/01_scatter_with_regression.png)
*图 1：广告投入与销售额的散点图加回归线，先看关系，再谈系数。*

![](images/02_coefficient_interpretation.png)
*图 2：系数解释示意，把斜率翻译成业务语言。*

> **课堂互动**：[OLS Line Lab：为什么这条线是“最好”的？](interactive/01_ols_line_lab.html)
>
> 学到 OLS 后打开它，让学生拖动散点，观察回归线、残差平方和和 R² 如何一起变化。重点不是背公式，而是看见"最小二乘"到底在最小化什么。
...
> - 交叉验证是防止过度拟合的核心方法：将数据分成 k 份，在 k-1 份上训练，在 1 份上验证（注意：交叉验证是下一周 Week 10 的内容，这里先了解概念即可）。
...
> **课堂互动**：[R² Prediction Trap：R² 高就一定好吗？](interactive/03_r2_prediction_trap.html)
>
> 放在 R² 和过度拟合侧栏之后演示。这里先让学生看到"样本内 R²"和"新数据表现"可能分开，正式的验证集、交叉验证和数据泄漏问题留到 Week 10。

---

## 2. 简单线性回归——解读系数与 R²

阿码盯着上一节的回归方程，突然发现问题："等等，这个方程里的 0.5 到底是什么意思？是说广告投入 1 万，销售额就涨 0.5 万吗？还是说……"

老潘笑了，这正是很多初学者会卡住的地方。"0.5 是斜率，但'斜率'这个词本身就挺让人困惑的。不如我们换个角度：把它翻译成决策者能听懂的话。"

他在黑板上写了一个方程：`sales = 10 + 0.5 × ad_spend`
...
小北若有所思："所以斜率告诉我'关系有多大'？"

"对。但要加限定：它说的是当前线性模型里的平均关系，不是自动证明因果影响。"老潘继续说，"斜率还有一个 p 值，告诉你这个关系是否显著不为 0。这和你 Week 06 学的假设检验是同一回事。"

### 回归系数的显著性检验

你在 Week 06 学过假设检验。回归分析中也有类似的检验：

- **零假设（H0）**：斜率 = 0（在线性模型中，x 与 y 没有线性关联）
- **备择假设（H1）**：斜率 ≠ 0（在线性模型中，x 与 y 有线性关联）

如果 p < 0.05，你拒绝 H0，认为"在当前模型假设下，x 与 y 的线性关联显著不为 0"。这仍然不是因果结论；因果结论需要实验设计或更明确的因果识别策略。
...
"好问题。"老潘笑了，"下节我会给你看一张残差图。你一眼就能看出'有问题'——但问题是什么，怎么处理，那才是关键。"

（下一节：当残差图告诉你"有问题"时，该怎么办？）

---

## 3. 回归假设——模型是有前提的

你跑完回归模型，看到 R² = 0.75，p < 0.001，心想："完美，可以写报告了。"

但这里有个陷阱：**在 AI 时代，任何人都可以在 5 秒内得到这些数字**。AI 会帮你拟合模型、计算 R²、输出 p 值。但它不会告诉你一件事：**这些数字基于假设，而假设可能不成立**。

小北上周就踩了这个坑。他拿着一份"完美"的回归结果去找老潘，R² 高达 0.82，所有系数都显著。

老潘只问了一个问题："残差图呢？"

"残差图？"小北愣住了，"我需要画那个吗？AI 没给我啊。"

老潘笑了笑："AI 给你的是'拟合结果'，不是'可信的结论'。残差图告诉你：这个模型的前提假设是否满足。如果假设不成立，你那个 0.82 的 R²……"

"会怎样？"

"会像一个看起来健康、但血压爆表的人。"老潘说，"外表正常，内部有问题。"
...
"好，现在来看看这些假设到底是什么。"老潘在黑板上写了四个字母——L-I-N-E。"记住这个词，你就记住了回归的四大假设。"
...
**如何加入二次项？** 创建一个新变量 x²，然后拟合模型 y = β0 + β1x + β2x²：

```python
# 假设原变量是 x
X_squared = X ** 2  # 创建二次项
X_with_square = np.column_stack([X, X_squared])  # 合并 x 和 x²
X_with_const = sm.add_constant(X_with_square)  # 添加截距
model = sm.OLS(y, X_with_const).fit()  # 拟合模型
```
...
> **课堂互动**：[Residual Diagnostics Lab：残差图诊断诊所](interactive/02_residual_diagnostics_lab.html)
>
> 讲完残差图和 QQ 图后打开它，让学生切换线性、非线性、异方差和高影响点场景，先口头判断问题，再看页面反馈。
...
你学会了画残差图。很棒——但当你看到图里有一个点离 0 特别远时，你会怎么办？

**第一反应可能是："删掉它，R² 会更好看。"**

停下。这个念头很危险。

老潘在公司里见过太多这样的例子：分析师为了"漂亮的模型"删除"碍眼"的数据点，结果决策者基于这个"优化后的模型"做了重要决定，后来发现真实情况和模型预测完全相反。

"删除数据点是所有分析决策里最需要谨慎的，"老潘说，"因为一旦删了，没人知道你删过什么——除非你诚实地报告。"

那你怎么判断"该不该删"？模型诊断会给你答案——不是凭直觉，而是有工具、有流程。
...
![](images/03_residuals_vs_fitted.png)
*图 3：残差 vs 拟合值图，先看有没有曲线或扇形。*
...
![](images/03_qq_plot.png)
*图 4：QQ 图示意，点是否贴近对角线比“漂亮”更重要。*
...
**第一步：判断"能不能删"**（按优先级检查）
1. **是否录入错误？** 如果数据明显不合理（如人的身高 300cm），修正或删除
2. **是否测量错误？** 如果测量过程有问题（如设备故障），删除并说明原因
3. **是否真实但极端？** 如果数据真实但极端（如收入超高的用户），**不能只为了提高模型拟合而删除**

**第二步：处理高影响点**
4. **删除后重新拟合**：比较删除前后的系数，看结论是否改变
   - 如果删除后结论基本不变 → 说明结果稳健，可以报告敏感性分析
   - 如果删除后结论完全改变 → 说明结果被这个点绑架，需要谨慎解释
5. **稳健回归**：使用对异常值不敏感的方法（如 RLM），而不是简单删除
6. **报告敏感性分析**：无论删除还是保留，都要报告"删除前"和"删除后"的结果
...
**如何解读？**
- p < 0.05：拒绝 H0，存在异方差
- p ≥ 0.05：不能拒绝 H0，暂未看到明显异方差证据；这不是"证明等方差成立"
...
![](images/04_cooks_distance.png)
*图 5：Cook's 距离图，找出那些会“拉动”整条回归线的点。*

小北若有所思："所以诊断的目的是……"

"是让你能自信地说：这个结论在哪些范围内是可信的，在哪些地方需要谨慎。"

阿码举手："那如果我加更多变量呢？两个 x 诊断会更复杂吗？"

老潘笑了："加变量不只是'更复杂'，它会带来全新的问题——比如两个变量'打架'，让你的模型系数变得完全不可信。下一节。"

（下一节：当自变量之间"打架"时——多重共线性和多元回归）

---

## 5. 多元回归——当相关因素不止一个时

到目前为止，你学的都是简单回归：**一个** x 预测 y。但现实世界很少这么简单。

销售额不只和广告投入有关——价格、促销、季节、竞争对手活动……这些变量都可能同时变化。如果你只看广告投入，你会漏掉很多重要信息。

"那我把所有因素都加进去？"阿码问，"越多越好？"

老潘摇头："恰恰相反。加多少、加什么，是有讲究的。有时候，加错东西会让模型变糟糕——这叫'多重共线性'，一个听起来复杂、但理解后会恍然大悟的问题。"

![](images/05_multiple_regression.png)
*图 6：多元回归示意，让多个因素同时进入模型并比较它们的边际作用。*
...
**简单线性回归**：y = β0 + β1x + ε（一个自变量 x）

**多元线性回归**：y = β0 + β1x₁ + β2x₂ + ... + βkxₖ + ε（多个自变量 x₁, x₂, ..., xₖ）

阿码问："多元回归的系数和简单回归的系数有什么区别？"

"好问题。"老潘说，"多元回归的系数是'控制其他变量后'的条件关联。这和简单回归的系数很不一样。"

举个例子：
- 简单回归：广告投入与销售额的样本内关系（b = 0.5）
- 多元回归：控制价格、促销后，广告投入与销售额的条件关系（b = 0.3）

"为什么系数变了？"小北问。

"因为广告投入和价格相关。"老潘说，"简单回归中，广告投入的系数可能混入了价格变化带来的关系。多元回归中，价格被放进模型单独控制，广告投入的系数变成'在价格和促销不变时的条件关联'。这就是'控制其他变量后'的含义。"
...
**为什么 VIF 叫"方差膨胀因子"？** 来看一个数值例子：

假设 `广告投入` 和 `促销力度` 的相关系数是 0.95（几乎同步变化）。你用它们来预测销售额：

| 情况 | 广告的系数 | 广告的标准误 | p 值 | VIF |
|------|-----------|-------------|------|-----|
| 无共线性（相关=0） | 0.50 | 0.10 | <0.001 | 1.0 |
| 高共线性（相关=0.95） | 0.50 | 0.45 | 0.27 | 10.3 |

看到了吗？系数本身没变，但**标准误膨胀了约 3.2 倍**（√10.3 ≈ 3.2），从 0.10 变成 0.45，导致原本"高度显著"的变量变得"不显著"。这就是"方差膨胀"的含义：**共线性让你的估计变得模糊，就像透过毛玻璃看东西**。

**公式卡**：
- 回归方程：`y = β0 + β1x + ε`
- 多元回归：`y = β0 + β1x1 + β2x2 + ... + βkxk + ε`
- VIF：`VIF_j = 1/(1-R_j²)`，其中 `R_j²` 是“用其他自变量解释第 j 个自变量”时得到的 R²
- Cook's D 直觉：如果一个点一删除就明显改变系数，它的 Cook's D 往往会偏大
...
> **课堂互动**：[Multicollinearity VIF Lab：自变量“打架”时发生什么？](interactive/04_multicollinearity_vif_lab.html)
>
> 讲完 VIF 后打开它。让学生调高两个自变量的相关性，观察 VIF 和系数稳定性如何变化，强调共线性主要伤害的是系数解释边界，不一定等于预测马上变差。

### 交互项：当关系依赖于另一个变量时

**交互项**（Interaction）是指一个变量和 y 的关系依赖于另一个变量。举个例子：广告投入与销售额的关系，可能依赖于价格——如果价格很高，同样的广告投入可能对应不同的销售额变化。
...
**如何解读交互项系数？**
- 如果交互项系数为负：ad_spend 与 y 的斜率随 price 增加而变小
- 如果交互项系数为正：ad_spend 与 y 的斜率随 price 增加而变大

阿码问："我什么时候应该加交互项？"

老潘给了两个标准：
1. **理论支持**：你有理由相信两个变量的关系会相互依赖
2. **图示支持**：画分组散点图，看不同组别的回归线是否平行

"交互项是个强大的工具，但不要滥用。"老潘说，"只有在理论或数据支持时才加，否则会增加模型复杂度，导致过度拟合。"

> **课堂互动**：[Interaction Terms Explorer：交互项不是“再加一个变量”](interactive/05_interaction_terms_explorer.html)
>
> 讲完交互项后打开它。让学生比较不同组别的斜率和截距，练习把 `x:group` 解释成"斜率是否随组别改变"，而不是简单说成"多加了一个变量"。
...
老潘的经验法则：
- **预测优先**：用验证集或交叉验证评估泛化能力（Week 10 会系统讲）
- **解释优先**：选假设诊断可接受、系数解释清楚、调整 R² 或信息准则表现合理的模型
- **简洁原则**：在拟合相近时，选更简单的模型（更少特征）

"记住：没有'最好的模型'，只有'最适合你的问题的模型'。"老潘说，"如果你的目标是预测，要看新数据表现；如果你的目标是解释，要优先保证诊断、变量选择和系数解释边界说得清楚。"
...
到目前为止，StatLab 已经有了一个"不确定性量化"的报告：点估计 + 置信区间 + Bootstrap + 置换检验。但这里有一个"看不见的坑"：我们在报告里只做了"比较差异"，但没有回答"这个差异和哪些变量有关"。

这正是本周"回归分析与模型诊断"派上用场的地方。**本周的 StatLab 进展，是将"检验差异"升级为"解释差异"**——从"有没有差异"到"哪些变量相关、条件关联有多大、模型是否可靠"。

让我们来写一个函数，做回归分析并输出诊断报告。这个函数会把本周学到的所有诊断工具（残差图、QQ图、Cook's距离、BP检验、VIF）整合到一起，生成一份"可审计"的回归报告：
...
def format_regression_report(diag_results, confidence=0.95):
    ...
    if norm['is_normal']:
        md.append(f"- 结论：p > 0.05，不能拒绝正态性假设；未发现明显偏离 ✅\n\n")
    else:
    ...
    if homo['is_homoscedastic']:
        md.append(f"- 结论：p > 0.05，不能拒绝同方差假设；未发现明显异方差证据 ✅\n\n")
    else:
...
| 上周 | 本周 |
|------|------|
| 比较差异（p 值 + CI） | 解释关系（回归系数 + 假设检查） |
| 检验"有没有差异" | 检验"x 与 y 如何相关" |
