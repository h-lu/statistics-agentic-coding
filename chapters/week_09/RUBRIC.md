...
| 系数解读准确性 | 5 分 | 正确解读截距和斜率的含义，并说明这是模型内关系而非自动因果结论 |
...
| 思考题回答 | 5 分 | 正确理解"高 R² 不等于好模型"，能提到假设诊断、解释边界或泛化评估限制 |
...
| 假设总结 | 3 分 | 对四个假设给出清晰判断，并避免把 p > 0.05 写成"证明假设成立" |
...
| "控制其他变量后"解释 | 7 分 | 清楚解释多元回归系数的条件关联含义、变化原因和非因果边界 |

**常见扣分点**：
- 不理解"控制其他变量后"的含义（-5 分）
- 只报告数字，不解释差异原因（-3 分）
- 混淆简单回归和多元回归系数（-2 分）
- 把"控制其他变量后"写成未经识别的因果影响（-3 分）
...
| 处理建议 | 4 分 | 建议合理（删除变量/PCA/正则化），并说明共线性主要影响系数解释稳定性 |
...
| HC3 原理解释 | 2 分 | 清楚解释 HC3 如何修正标准误，避免说成修复数据中的异方差 |
...
本周提供两类测试：一类验证示例代码和参考实现能否运行，另一类用固定数据检查部分统计计算。运行以下命令验证：

```bash
python3 -m pytest chapters/week_09/tests/test_assignment.py -v
python3 -m pytest chapters/week_09/tests/test_solution.py -v
```
...
**注意**：当前自动测试主要面向示例代码、参考实现和部分数值计算；学生最终成绩仍应以本 Rubric 的人工评分为准。自动测试不能替代对解释边界、诊断图、可视化和报告质量的人工判断。
...
- "多元回归系数解读清楚，能把'控制其他变量后'说成条件关联，而不是直接因果影响。"
