...
-->
<!-- code_block_exemption: 本章同时提供 SciPy 基础路径与 PyMC 选读路径，代码块较多但都服务于同一主题。 -->
...
```mermaid
flowchart TD
    A[你要做的是重复实验吗？] -->|是| B[优先频率学派]
    A -->|否 / 单次决策 / 需要概率化表达| C[优先贝叶斯学派]
    B --> D[更关心 p 值、置信区间、长期频率]
    C --> E[更关心后验概率、可信区间、Expected Loss]
    D --> F[把假设检验和效应量写清楚]
    E --> F
```
...
> 如果假阳性成本更高，你应该把阈值调高，让决策更保守；如果假阴性成本更高，你应该把阈值调低，让决策更积极。**阈值不是固定常数，而是由业务代价决定的**。
