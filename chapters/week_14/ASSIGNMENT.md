...
4. 如果假阳性成本和假阴性成本不一样，说明你会如何调整决策阈值
5. 用业务语言向非技术读者解释结果
...
公式：Expected Loss = E[max(θ_A - θ_B, 0) | data]
实现：从两个后验分布中采样，计算当 θ_A > θ_B 时 (θ_A - θ_B) 的均值

阈值如何变化？
- 如果假阳性成本更高（比如推错版本会造成投诉或退款），阈值应该更高，推全更保守
- 如果假阴性成本更高（比如错过更好的版本会损失大量转化），阈值应该更低，推全更积极
