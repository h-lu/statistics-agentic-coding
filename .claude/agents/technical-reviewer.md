...
检查：
- **每节代码块数**：是否超过 2 个？超过则标记为 S2
- **全章代码块数**：是否超过 10 个（不含 StatLab）？超过则标记为 S2
- **代码块长度**：是否超过 30 行？超过则标记为 S3，建议精简或移入 examples/
- **代码必要性**：每个代码块是否都有教学必要？是否有重复演示？
- **代码位置**：示例代码是否应移入 `examples/` 目录，正文只保留核心片段？

**代码泛滥判定**：
| 情况 | 严重级别 | 处理建议 |
|------|---------|---------|
| 单节 > 2 个代码块 | S2 | 合并或移入 examples/ |
| 全章 > 10 个代码块 | S2 | 精简，核心保留，其他移入 examples/ |
| 单块 > 30 行 | S3 | 精简或移入 examples/ |
| 连续 > 2 个代码块无文字 | S2 | 补充解释文字 |
