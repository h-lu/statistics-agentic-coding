# Agent Instructions (statistics-agentic-coding)

本仓库用于生成《统计学与 Agentic 数据分析》教材的"每周章包"交付物（见 `chapters/`）。
...
| `example-engineer` | 示例代码 + 反例 + StatLab 超级线代码 | 与贯穿案例关联 |
...
| StatLab 演进连续 | example-engineer | 在上周基础上增量修改（报告/流水线不从头写） |
